...
export class UniversityBusScheduleCrawlerClient implements CrawlerClient {
	...
	async crawl(): Promise<any> {
		...
		try {
			...
			const schoolToStation: UniversityBusSchedule[] = [];
			await Promise.all(
			...
			const stationToSchool: UniversityBusSchedule[] = [];
			await Promise.all(
			...
			const toSchools: UniversityBusSchedule[] = [];
			// 등교
			...
			const fromSchools: UniversityBusSchedule[] = [];
