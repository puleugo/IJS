...
export class UniversityService {
	...
	async getUniversityProgramsByDate(
		...
		return await this.universityProgramRepository.find({ where: { endAt: MoreThan(date), }, });
	}
	...
	async getUniversityFinishDate(
		...
		if (!semester) {
			const comingSemester = await this.getUniversityComingSemesterByDate(
				date
			);
			if (!comingSemester)
	...
	async getUniversityBusInfo(): Promise<UniversityBusResponseType> {
		const toSchoolBus = await this.universityBusScheduleRepository.find({ where: { toSchool: true, }, });
		const fromSchoolBus = await this.universityBusScheduleRepository.find({ where: { fromSchool: true, }, });
	...
	async getMajors():Promise<UniversityMajor[]> {
	...
	async getUniversityNextBusInfo(
		...
		const busInfo = await this.universityBusScheduleRepository.find({
			...
			order: { departedOn: 'ASC', },
		});
		busInfo.map(
			(bus) => {
				return (bus.departedOn = getDateByTime(bus.departedOn.toString()));
			}
		);

		return busInfo.filter((bus) => {
			return bus.departedOn > currentTime;
		});
	}
	...
	async getUniversityNotices(): Promise<
        UniversityNoticePreviewResponseType[]
        > {
		return await this.universityNoticeRepository.find({ select: { id: true, }, });
	}
	...
	private async getUniversityMealInfoByToday(
		...
		const courseA = meals.find((meal) => {
			return meal.course === MealCourseEnum.A;
		});
		const courseB = meals.find((meal) => {
			return meal.course === MealCourseEnum.B;
		});
		const courseC = meals.find((meal) => {
			return meal.course === MealCourseEnum.C;
		});
	...
	private async getUniversityMealInfoByWeekly(
		...
		const meals = await this.universityMealRepository.find({
			where: { publishedAt: Between(lastMonday, thisFriday), },
			order: { publishedAt: 'ASC', },
		});

		if (meals.length === 0)
			throw new NotFoundException('해당 날짜의 식단 정보가 없습니다.');
		const courseA = meals.filter((meal) => {
			return meal.course === 'A';
		});
		const courseB = meals.filter((meal) => {
			return meal.course === 'B';
		});
		const courseC = meals.filter((meal) => {
