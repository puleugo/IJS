...
import { UniversityMealSearchQuery, } from '@app/university/dto/university.type';
...
export class UniversityController {
    ...
    @Get('programs')
    @ApiOperation({ summary: '신청할 수 있는 비교과 프로그램 목록을 가져옵니다.', })
    @ApiOkResponse({
    	type: UniversityProgramProfileResponse,
    	isArray: true,
    })
    async getUniversityPrograms(): Promise<UniversityProgramProfileResponse[]> {
    	const programs =
            await this.universityService.getUniversityProgramsByDate(
            	new Date()
            );

    	return programs.map(
    		(program) => {
    			return new UniversityProgramProfileResponse(program);
    		}
    	);
    }

    @Get('majors')
    @ApiOperation({ summary: '학과 목록을 가져옵니다.', })
    @ApiOkResponse({
    	type: UniversityMajorProfileResponse,
    	isArray: true,
    })
    async getMajorProfiles(): Promise<UniversityMajorProfileResponse[]> {
    	...
    	return majors.map((major) => {
    		return new UniversityMajorProfileResponse(major);
    	});
    }
    ...
    @ApiOkResponse({
    	type: UniversityNoticePreviewResponse,
    	isArray: true,
    })
    async getUniversityNotices(): Promise<UniversityNoticePreviewResponse[]> {
    	...
    	return notices.map(
    		(notice) => {
    			return new UniversityNoticePreviewResponse(notice);
    		}
    	);
    }
    ...
    @ApiOkResponse({
    	type: UniversityNoticeProfileResponse,
    	isArray: true,
    })
    async getUniversityNotice(
    	...
    	const notices =
            await this.universityService.getUniversityNoticeProfileById(id);

    	return notices.map(
    		(notice) => {
    			return new UniversityNoticeProfileResponse(notice);
    		}
    	);
    }
    ...
    @ApiOperation({
    	...
    	externalDocs: { url: 'https://www.inje.ac.kr/kor/Template/Bsub_page.asp?Ltype=4&Ltype2=1&Ltype3=0&Tname=S_Schedule&Ldir=board/S_Schedule&Lpage=Tboard_L&d1n=4&d2n=2&d3n=1&d4n=1', },
    })
    @ApiResponse({ type: UniversityCalendarResponse, })
    async getUniversityCalendarInfo(): Promise<UniversityCalendarResponse> {
    	const calendarInfo =
            await this.universityService.getUniversityCalendarInfo(new Date());

    	return new UniversityCalendarResponse(calendarInfo);
    }

    @Get('next-bus-info')
    @ApiOperation({ summary: '금일 학교에서 아직 탈 수 있는 버스 정보를 가져옵니다.', })
    @ApiOkResponse({
