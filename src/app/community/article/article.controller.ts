...
export class ArticleController {
	...
	async getArticles(
		...
		if (board.isCouncil) {
			const articles =
                await this.articleService.getPaginatedCouncilArticles({
                	boardId,
                	page,
                	limit,
                });
    ...
    async createArticle(
        @Param('boardId', ParseIntPipe) boardId: number,
        @Body() articleCreateRequest: ArticleCreateRequest | CouncilArticleCreateRequest,
        @Req() { user, }: Request
    ): Promise<ArticleProfileResponse> {
    	const board = await this.boardService.findById(boardId);
    	if (!board) throw new BoardNotFoundException();
    ...
    async likeArticle(
    	...
    	if (article.authorId === user.id)
    		throw new CanNotLikeOwnArticleException();
    ...
    @ApiCreatedResponse({ description: '게시글 신고 성공', })
    async reportArticle(
    ...
    @ApiCreatedResponse({ description: '게시글 수정 성공', })
    @ApiNotFoundResponse({
    	description: [
    		'게시글을 찾을 수 없습니다.', '게시판을 찾을 수 없습니다.',
    	].join('<br>'),
    })
    @ApiBadRequestResponse({ description: '게시글에 대한 권한이 없습니다.', })
    ...
    @ApiCreatedResponse({ description: '게시글 삭제 성공', })
    @ApiNotFoundResponse({
    	description: [
    		'게시글을 찾을 수 없습니다.', '게시판을 찾을 수 없습니다.',
    	].join('<br>'),
    ...
    async deleteArticle(
    	...
    	const article = await this.articleService.findById(id, { where: { boardId: board.id, }, });
    	if (!article) throw new ArticleNotFoundException();
    	if (article.authorId !== user.id)
    		throw new ArticlePermissionDeniedException();
