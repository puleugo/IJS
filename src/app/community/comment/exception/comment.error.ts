...
export class CantLikeOwnCommentException extends BadRequestException {
	constructor() {
		super(
			'자신의 댓글에는 좋아요를 누를 수 없습니다.', COMMENT_ERRORS.CANT_LIKE_OWN_COMMENT
		);
	}
}
...
export class AlreadyLikedCommentException extends BadRequestException {
	constructor() {
		super(
			'이미 좋아요가 눌려져있습니다.', COMMENT_ERRORS.ALREADY_LIKED_COMMENT
		);
	}
}
...
export class CommentPermissionDeniedException extends BadRequestException {
	constructor() {
		super(
			'댓글에 대한 권한이 없습니다.', COMMENT_ERRORS.COMMENT_PERMISSION_DENIED
		);
	}
}
...
export class FailedToLikeCommentException extends BadRequestException {
	constructor() {
		super(
			'댓글에 좋아요를 누를 수 없습니다.', COMMENT_ERRORS.FAILED_TO_LIKE_COMMENT
		);
	}
}
